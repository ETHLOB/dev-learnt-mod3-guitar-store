...
import { type guitarraType } from "@/types/guitarra";
import { computed } from "vue";

const props = defineProps<{
  cart: Array<guitarraType>;
  headerGuitar: guitarraType;
}>();

const emit = defineEmits<{
  "add-to-cart": [guitarra: guitarraType];
  "remove-from-cart": [id: number];
  "clean-cart": [];
  "increase-product-amount": [id: number];
  "decrease-product-amount": [id: number];
}>();
        ...
        >
          <div class="carrito">
            ...
            <div id="carrito" class="bg-white p-3">
              <p v-if="props.cart.length === 0" class="text-center m-0">
              ...
              <div v-else>
                <table class="w-100 table">
                  ...
                  <tbody>
                    <tr v-for="product in props.cart" :key="product.id">
                      <td>
                        <img
                          class="img-fluid"
                      ...
                      <td class="flex align-items-start gap-4">
                        <button
                          class="btn btn-dark"
                          type="button"
                          @click="emit('decrease-product-amount', product.id)"
                        >
                          -
                        </button>
                        {{ product.cantidad }}
                        <button
                          type="button"
                          class="btn btn-dark"
                          @click="emit('increase-product-amount', product.id)"
                        >
                          +
                        </button>
                      </td>
